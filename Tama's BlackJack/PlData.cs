...
namespace Tama_s_BlackJack
{
    class PlData
    {
        ...
        private readonly int[] pointWidth = new int[] { 4000, 4000, 4000 };
        private readonly String[] evalP = new String[] { "散財のプロ", "負け好き", "凡人", "運の良い人", "強運の持ち主", "やばい人", "廃人", "改造" };

        public String GetEvalOfPoint()
        {
            
            int avg = (int)this.GetMaxOfPoint();
            if (avg / pointWidth[this.GetNowGameMode() - 1] >= evalP.Length)
            {
                return evalP[evalP.Length - 1];
            }
            else
            {
                return evalP[avg / pointWidth[this.GetNowGameMode() - 1]];
            }
        }

        public String GetEvalOfPoint(int mode)
        {
            if (mode > modeNum || mode < 1)
            {
                throw new IndexOutOfRangeException();
            }
            int avg = (int)this.GetMaxOfPoint(mode);
            if(avg / pointWidth[mode - 1] >= evalP.Length)
            {
                return evalP[evalP.Length - 1];
            }
            else
            {
                return evalP[avg / pointWidth[mode - 1]];
            }
        }

        private readonly int[] tScoreWidth = new int[] { 400, 400, 400 };
        private readonly String[] evalT = new String[] { "未プレイ", "たまご", "ひよこ", "凡人", "経験者", "運がいい人", "やばい人" };

        public String GetEvalOftScore()
        {
            int avg = (int)this.GetAvgOftScore();
            if (avg / tScoreWidth[this.GetNowGameMode() - 1] >= evalT.Length)
            {
                return evalT[evalT.Length - 1];
            }
            else
            {
                return evalT[avg / tScoreWidth[this.GetNowGameMode() - 1]];
            }
        }

        public String GetEvalOftScore(int mode)
        {
            if (mode > modeNum || mode < 1)
            {
                throw new IndexOutOfRangeException();
            }
            int avg = (int)this.GetAvgOftScore(mode);
            if (avg / tScoreWidth[mode - 1] >= evalT.Length)
            {
                return evalP[evalP.Length - 1];
            }
            else
            {
                return evalP[avg / tScoreWidth[mode - 1]];
            }
        }
